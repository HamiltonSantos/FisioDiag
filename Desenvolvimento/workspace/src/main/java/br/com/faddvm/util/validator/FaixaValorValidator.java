...
public class FaixaValorValidator implements Validator {
	...
	private boolean validaFaixa(FaixaValor f, Errors e) {
		...
		if (f.getPeso() == null) {
			e.reject(null, "Peso ï¿½ obrigatï¿½rio");
			return true;
		}

		if (f.getValorMin() == null) {
			e.reject(null, "Valor mï¿½nimo ï¿½ obrigatï¿½rio.");
			return true;
		}

		if (f.getValorMax() == null) {
			e.reject(null, "Valor mï¿½ximo ï¿½ obrigatï¿½rio.");
			return true;
		}

		if (f.getValorMin() > f.getValorMax()) {
			e.reject(null,
					"Valor mï¿½nimo deve ser menor ou igual a valor mï¿½ximo.");
			return true;
		}

		if (f.getValorMin() < 0 || f.getValorMax() < 0) {
			e.reject(null, "Valor mï¿½nimo deve ser maior ou igual a 0");
			return true;
		}
		if (f.getValorMax() < 0) {
			e.reject(null, "Valor mï¿½ximo deve ser maior ou igual a 0");
			return true;
		}
		if (f.getPeso() < 0) {
		...
		if (f.getValorMin() > 100000 || f.getValorMax() > 100000) {
			e.reject(null, "Valor mï¿½nimo deve ser menor ou igual a 100000");
			return true;
		}
		if (f.getValorMax() > 100000) {
			e.reject(null, "Valor mï¿½ximo deve ser menor ou igual a 100000");
			return true;
		}
		if (f.getPeso() > 100000) {
		...
		if (f.getDescricao() == null) {
			e.reject(null, "Descriï¿½ï¿½o ï¿½ obrigatï¿½ria.");
			return true;
		}
		f.setDescricao(f.getDescricao().trim());
		if (f.getDescricao().length() < 3 || f.getDescricao().length() > 250) {
			e.reject(null, "Descriï¿½ï¿½o deve ter 3 caracteres ou mais");
			return true;
		}
	...
	private boolean validaRange(FaixaValor faixa, Errors errors) {
		...
		if (vIniciada && vTipo == 'R') {
			if (faixa.getValorMax() > vValorMax
				...
				errors.reject(null,
						"Faixa Inv‡lida, valor mï¿½nimo pode iniciar em "
								+ (vValorMax + 1));
				return true;
			}
			...
			if (faixa.getValorMin() < vValorMin
				...
				errors.reject(null,
						"Faixa Invï¿½lida, valor Mï¿½ximo pode terminar em "
								+ (vValorMin - 1));
				return true;
			}
			...
			if (faixa.getValorMax() == vValorMax
				...
				errors.reject(null, "Vocï¿½ nï¿½o pode criar essa Faixa");
				return true;
			}
